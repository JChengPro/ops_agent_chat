...
- 审批 Hash 绑定能力版本、参数、运行时、目录、连接目标和注册配方的不可变快照，批准后修改环境配置不会改变实际执行内容。
- 服务启动、停止、重启和扩缩容需要人工审批；执行后验证采用 fail-closed，失败时按前置状态自动恢复或明确报告无法恢复。
- 未注册的删除资源、任意 Shell 等破坏性能力不可通过审批临时放行。
- Runtime Evidence、Tool Invocation、Model Call、Agent Step、原子 Claim/Evidence 关联和可校验的链式审计。
...
真实 Docker Compose Adapter 测试默认跳过；在具备 Docker 的主机上使用 `RUN_DOCKER_INTEGRATION=1 pytest -q tests/test_docker_runtime_integration.py`。仓库 CI 会执行迁移往返、Registry 编译、后端测试、真实 Docker Adapter、前端测试、前端构建和 Compose 镜像构建。
...
- 审批绑定 Action hash、目标、环境、能力版本、参数、连接和最终执行配方；任一审批内快照变化都会使审批失效。
- Action 通过数据库条件更新从 `approved/ready` 原子进入 `executing`，已经成功或进入终态的 Action 不能重复执行。
- 所有变更能力必须注册只读前置检查与验证器；未知验证规则默认失败。
