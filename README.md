...
- 登录页支持登录与自助注册切换；新注册账号固定为普通用户，项目和聊天记录按用户隔离。部署者可以关闭注册或要求输入注册码。
...
- React 三栏工作台：项目与会话、聊天与审批、活动/经验/配置；左右侧栏可独立收起，控制按钮固定在视口中部，不随列表滚动。活动页将“主动巡检”和“对话 Agent 活动”分区展示，避免自动任务与用户对话混在一起。
- 新建项目和后续编辑使用统一配置弹窗，必填项使用 `*` 标记，并提供可展开的 SSH 配置指南。右侧只展示当前项目摘要，SSH Connection 不跨项目混合展示。
...
- 同一环境、服务和问题类型的持续异常会更新现有事件，不会每轮创建重复告警；后续巡检确认恢复时，同一事件会进入 `resolved` 并记录恢复时间。
- 工作台根据事件状态快照刷新通知。修复失败后如果下一轮确认已经恢复，新的恢复通知会替换旧的失败提示；仅发生巡检次数变化不会反复弹窗，历史已恢复事件也不会在首次打开页面时产生过期提示。

严重级别为 `critical` 的活动事件还会触发一条独立的自动诊断链路：

```text
critical MonitorEvent
  -> 创建且只创建一个隐藏的只读 AgentRun
  -> 服务级事件确定性读取 service.status 和最多 100 行 service.logs
  -> 环境级事件重新读取 service.list
  -> LLM 基于实时 Evidence 生成影响、可能原因、证据缺口和处理建议
  -> 诊断结果回写 MonitorEvent，并在工作台“活动”区持续展示
  -> 用户确认建议后，可将其带入普通聊天生成受 Policy 管理的修复方案
```

自动诊断 Run 只会获得 Registry 中 `effect=read` 的能力。即使模型错误地提出停止、重启或修改操作，Graph 也会拒绝该调用，不创建 Approval，更不会执行变更。诊断建议本身不等于修复；用户后续发送修复请求后，仍需依次经过 Action 快照、Policy、必要的人工审批、执行和 Verification。

工作台右侧的“活动”页将上述 `MonitorEvent` 放在“主动巡检”区域，将用户聊天产生的 `AgentRun` 放在“对话 Agent 活动”区域。两类记录仍可分别查看状态与详情，但不会再共用一条无差别时间线。
...
# 本地开发可以留空；对外或生产部署必须使用至少 16 位随机注册码
...
`ADMIN_*` 只用于首次启动时创建初始管理员，不是所有用户共用的账号。自助注册用户的角色固定为 `user`，不会自动获得管理员身份、其他用户的聊天记录或已有项目权限。设置 `REGISTRATION_ENABLED=false` 可以关闭注册入口；设置 `REGISTRATION_INVITE_CODE` 后，注册表单会要求填写对应注册码。生产环境如果开启注册，后端会强制要求非默认且至少 16 位的注册码，否则拒绝启动。
...
| `REGISTRATION_ENABLED` | 是否允许登录页创建新账号 |
| `REGISTRATION_INVITE_CODE` | 可选注册码；生产环境开放注册时必须设置至少 16 位 |
...
- 新注册用户固定为普通用户，用户名和邮箱不区分大小写且必须唯一，密码使用带随机盐的 PBKDF2 哈希保存；注册不会授予已有项目访问权。
- 生产环境不应开放无注册码的自助注册；不需要多用户时应设置 `REGISTRATION_ENABLED=false`。
