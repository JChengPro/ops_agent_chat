...
export function WorkspacePage({user,onLogout}:{user:User;onLogout:()=>void|Promise<void>}) {
  ...
  useEffect(()=>{const environmentId=activeEnvironment?.id;if(!projectId||!environmentId){setMonitorEvents([]);return;}let disposed=false,timer:number|undefined,initialized=false;const snapshots=new Map<string,string>();async function refresh(){try{const [events,latestEnvironments]=await Promise.all([listMonitorEvents(projectId!,environmentId),listEnvironments(projectId!)]);if(disposed)return;setMonitorEvents(events);setEnvironments(latestEnvironments);const visible=events.find(item=>shouldNotifyMonitorEvent(item,snapshots.get(item.id),initialized));for(const item of events)snapshots.set(item.id,monitorEventSnapshot(item));initialized=true;if(visible){const next=monitorNoticeFor(visible);showNotice(next.kind,next.text);}}catch(error){if(!disposed)showNotice("error",error instanceof Error?error.message:"主动巡检状态加载失败");}finally{if(!disposed)timer=window.setTimeout(refresh,5000);}}void refresh();return()=>{disposed=true;if(timer)window.clearTimeout(timer);};},[projectId,activeEnvironment?.id]);
