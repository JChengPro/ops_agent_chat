...
export function WorkspacePage({user,onLogout}:{user:User;onLogout:()=>void|Promise<void>}) {
  ...
  useEffect(()=>{const environmentId=activeEnvironment?.id;if(!projectId||!environmentId){setMonitorEvents([]);return;}let disposed=false,timer:number|undefined;async function refresh(){try{const [events,latestEnvironments]=await Promise.all([listMonitorEvents(projectId!,environmentId),listEnvironments(projectId!)]);if(disposed)return;setMonitorEvents(events);setEnvironments(latestEnvironments);const diagnosed=events.find(item=>item.diagnosis_summary&&["completed","failed","cancelled"].includes(String(item.diagnostic_run_status)));const visible=diagnosed||events.find(item=>!["resolved"].includes(item.status));if(visible){const token=`${visible.id}:${visible.occurrence_count}:${visible.status}:${visible.diagnostic_run_status||"none"}:${visible.diagnosed_at||"none"}`;if(!seenMonitorEvents.current.has(token)){seenMonitorEvents.current.add(token);if(visible.diagnosis_summary)showNotice(visible.diagnostic_run_status==="completed"?"info":"error",visible.diagnostic_run_status==="completed"?`自动只读诊断已完成：${visible.summary}`:"自动只读诊断未成功，请查看主动巡检事件");else showNotice(visible.status==="remediated"?"success":"error",visible.summary);}}}catch(error){if(!disposed)showNotice("error",error instanceof Error?error.message:"主动巡检状态加载失败");}finally{if(!disposed)timer=window.setTimeout(refresh,5000);}}void refresh();return()=>{disposed=true;if(timer)window.clearTimeout(timer);};},[projectId,activeEnvironment?.id]);
