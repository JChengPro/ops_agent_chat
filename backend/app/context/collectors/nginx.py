...
class NginxCollector:
    ...
    def collect(self, db, environment, connection, run=None):
        run = begin_collector_run(db, environment, self.name, run); entity_count = relationship_count = 0
        try:
            ...
            for path in environment.config_json.get("nginx_config_files") or []:
                content = self.transport.read_file(connection, environment, str(path), cancel_check=lambda: collector_cancelled(run.id)); source = db.scalar(select(ContextSource).where(ContextSource.project_id == environment.project_id, ContextSource.environment_id == environment.id, ContextSource.source_type == "nginx", ContextSource.source_ref == str(path)))
            ...
            run.summary_json = {"entities": entity_count, "relationships": relationship_count}; run.error_message = None
