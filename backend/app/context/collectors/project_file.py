...
class ProjectFileCollector:
    ...
    def collect(self, db, environment, connection, run=None):
        run = begin_collector_run(db, environment, self.name, run); totals = {"entities": 0, "relationships": 0, "files": 0}
        try:
            for path in environment.config_json.get("context_files") or []:
                content = self.transport.read_file(connection, environment, str(path), cancel_check=lambda: collector_cancelled(run.id)); payload = json.loads(content) if str(path).endswith(".json") else yaml.safe_load(content)
                ...
                totals["files"] += 1
            run.summary_json = totals; run.error_message = None
